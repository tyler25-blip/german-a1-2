  ...
  const renderPage = async (root) => {
    ...
    const summaryHtml = `
      <div class="progress-summary">
        ...
        <div class="stat-card"><span class="num">${accuracy}%</span><span class="label">整體答對率</span></div>
      </div>
    `;

    // 為了知道每章總題數，需要 fetch 各章 JSON。為了快，只 fetch 已有作答記錄的章節。
    const chapterRows = await Promise.all(index.map(async ch => {
      ...
      return `
        <a href="./chapter.html?ch=${ch.id}" class="progress-row ${klass}">
          <span class="num-pill">L ${ch.id}</span>
          <span>
            <span class="ch-title">${App.escapeHtml(ch.title_zh)}</span>
            <span class="muted small"> · ${App.escapeHtml(ch.title_de)}</span>
            <br><span class="ch-stats">${stats}</span>
          </span>
    ...
    root.innerHTML = `
      ...
      ${summaryHtml}
      <h2>各章節進度</h2>
      <div class="progress-list">${chapterRows.join('')}</div>
